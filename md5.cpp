...
const byte* MD5::getDigest() {
  if (!finished) {
    ...
    byte bits[8];  //unsigned char bits[8]
    bit32 oldState[4];
    bit32 oldCount[2];
    bit32 index, padLen;

    /* Save current state and count. */
    /*void* memcpy( void *dest, const void *src, size_t count );   count characters */
    ...
    padLen = (index < 56) ? (56 - index) : (120 - index);  //超过56个字节的话，就不能用56减，就要用 56+64=120来减，64 mod 64 为0，还是算的56
    init(PADDING, padLen);//init byte MD5::PADDING[64] = { 0x80 };  padlen=53
 ...
 *
 */
void MD5::init(const byte* input, size_t len) { //byte unsigned char  size_t 为64bit  改为quint64稳当一些
    //设msg 为abc  则len为3
    //init(byte [64] = { 0x80 }, 53);//init byte MD5::PADDING[64] = { 0x80 };  padlen=53

  bit32 i, index, partLen;   //unsigned int

  finished = false;  //finished 成员

  /* Compute number of bytes mod 64   count[0]为bit值,>>3即除以8，得到byte值，&0x3f 得到mod64 余下的byte*/
  index = (bit32)((count[0] >> 3) & 0x3f);

  /* update number of bits   len<<3 即len*8 得到bit值 */
  if ((count[0] += ((bit32)len << 3)) < ((bit32)len << 3)) {
      //越界，超过了32bit
    ++count[1];
  }
  count[1] += ((bit32)len >> 29);  //len>>32 从64的高32到64的低32，len是byte值，还要乘8才是bit值，因此
                                  //还需左移3，所以一共右移29

  partLen = 64 - index;  //512bit即为64byte，现在余下index byte， 需补上partLen byte

  /* transform as many times as possible. */
  if (len >= partLen) {

    memcpy(&buffer[index], input, partLen);  //unsigned char buffer[64]
...
void MD5::transform(const byte block[64]) {
  ...
  bit32 a = state[0], b = state[1], c = state[2], d = state[3], x[16];  //每次round都是32位
